...
8. **Validate final workpaper.** Use `scripts/validate_return.py`. A `VERIFIED` field must reference a rule in `rules/<year>/` with verified provenance, reviewed source values whose `original_text` literally contains the extracted number, and a reproducible calculation whose result equals the value. The rule's `application` block fixes the operation, the fields it applies to and the limits: the workpaper cannot choose them, and a per-unit cap multiplier must point to a source value with `times_input` (never a literal count). Claim each person/jurisdiction/field once: combine several documents with `sum` in one final field. Supported calculations: `identity`/`sum`/`difference` (with a mandatory `cap` when the rule defines a limit), `rule_value_times` (count x rule amount) or `percent_clamped` (percentage with minimum/maximum). Amounts above a legal maximum are rejected. Anything else (thresholds on net income, two-earner formula, depreciation) stays `REVIEW_REQUIRED`. Empty or unresolved workpapers are `REVIEW_REQUIRED`.
